...
class SLRModel ( nn.Module ) :
    ...
    def criterion_calculation ( self , ret_dict , label , label_lgt ) :
        ...
        for k , weight in self.loss_weights.items ( ) :
            if k == 'SeqCTC' :
                total_loss [ 'SeqCTC' ] = weight * self.loss [ 'CTCLoss' ] ( ret_dict [ "sequence_logits" ] [ 0 ].log_softmax ( -1 ) ,
                                                           label.cpu ( ).int ( ) ,
                ...
                if 'ConvCTC' in self.loss_weights :
                    total_loss [ f'ConvCTC_{i}' ] = weight * self.loss_weights [ 'ConvCTC' ] * self.loss [ 'CTCLoss' ] (
                ...
                total_loss [ 'ConvCTC' ] = weight * self.loss [ 'CTCLoss' ] ( ret_dict [ "conv_logits" ] [ 0 ].log_softmax ( -1 ) ,
                                                           label.cpu ( ).int ( ) ,
                                                           ret_dict [ "feat_len" ].cpu ( ).int ( ) ,
                                                           label_lgt.cpu ( ).int ( ) ).mean ( )
