    ...
    def criterion_calculation ( self , ret_dict , label , label_lgt ) :
        ...
        for k , weight in self.loss_weights.items ( ) :
            if k == 'SeqCTC' :
                ...
                loss += weight * self.loss_weights [ 'SeqCTC' ] * self.loss [ 'CTCLoss' ] (
                    ret_dict [ "sequence_logits" ] [ i ].log_softmax ( -1 ) ,
                    label.cpu ( ).int ( ) , ret_dict [ "feat_len" ].cpu ( ).int ( ) ,
                    label_lgt.cpu ( ).int ( ) ).mean ( )
                if 'ConvCTC' in self.loss_weights :
                    loss += weight * self.loss_weights [ 'ConvCTC' ] * self.loss [ 'CTCLoss' ] (
                ...
                if 'Dist' in self.loss_weights :
                    ...
                    #                                                                     use_blank=False)
                    loss += weight * self.loss_weights [ 'Dist' ] * self.loss [ 'distillation' ] (
